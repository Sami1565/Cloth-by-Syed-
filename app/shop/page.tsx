...
export default function Shop() {
  ...
  const getCategoryIcon = (category: string) => {
    const icons: { [key: string]: string } = {
      'All': '🛍️', 'Men': '👔', 'Women': '👗', 'Kids': '🧒',
      'Accessories': '👜', 'Unstitched': '🧵', 'Embroidered': '🌸',
      'New In': '✨', 'Sale': '🏷️'
    }
  ...
  const getSubCategoryIcon = (sub: string) => {
    const icons: { [key: string]: string } = {
      'Jackets': '🧥', 'Pants': '👖', 'Shirts': '👔', 'Sweaters': '🧶',
      'T-Shirts': '👕', 'Blazers': '🤵', 'Dresses': '👗', 'Tops': '👚',
      'Traditional': '🧕', 'Bottoms': '👖', 'Shoes': '👟', 'Belts': '🔗',
      'Bags': '👜', 'Watches': '⌚', 'Eyewear': '👓', 'Wallets': '💳',
      'Scarves': '🧣', 'Fabrics': '🧵', 'Kurtas': '👘', 'Shawls': '🧣',
      'Dupattas': '🧣'
    }
    return icons[sub] || '📦'
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              <FaShoppingBag className="text-white/40 hover:text-[#d4af37] transition cursor-pointer text-xl" />
